...
retry:
		...
		Log(LOG_INFO, "\x1B[36mTotal Hashrate: %.02fH/s\x1B[0m\n", TotalHashrate);
		...
		while(strchr(rawresponse + bufidx, '\n'))
		{
			...
			if(msgid && json_integer_value(msgid))
			{
				...
				if(!authid)
				{
					...
					{
						Log(LOG_INFO, "\x1B[32mShare accepted: %d/%d (%.02f%%)\x1B[0m", GlobalStatus.SolvedWork - GlobalStatus.RejectedWork, GlobalStatus.SolvedWork, (double)(GlobalStatus.SolvedWork - 
GlobalStatus.RejectedWork) / GlobalStatus.SolvedWork * 100.0);
					}
					else
					{
						const char *errmsg;
						GlobalStatus.RejectedWork++;
						errmsg = json_string_value(json_object_get(err, "message"));
						Log(LOG_INFO, "\x1B[31mShare rejected (%s): %d/%d (%.02f%%)\x1B[0m", errmsg, GlobalStatus.SolvedWork - GlobalStatus.RejectedWork, GlobalStatus.SolvedWork, 
(double)(GlobalStatus.SolvedWork 
- GlobalStatus.RejectedWork) / GlobalStatus.SolvedWork * 100.0);
				...
				else
				{
					...
					JobIdx++;
					NextJob = &Jobs[JobIdx&1];
					Log(LOG_NOTIFY, "\x1B[35mNew job at diff %g\x1B[0m", (double)0xffffffff / CurrentJob->XMRTarget);
				}
				json_decref(result);
			}
			else

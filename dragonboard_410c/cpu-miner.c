...
static void share_result(int result, struct work *work, const char *reason) {
    ...
    case ALGO_CRYPTONIGHT:
        applog(LOG_INFO, "\x1B[32mSubmitted: %lu/%lu\x1B[0m",
                accepted_count, accepted_count + rejected_count);
        applog(LOG_INFO, "\x1B[36mHashrate: %.2f H/s\x1B[0m", hashrate);
        break;
    default:
...
static void *miner_thread(void *userdata) {
    ...
    while (1) {
        ...
        if (!opt_quiet) {
            switch(opt_algo) {
            case ALGO_CRYPTONIGHT:
                applog(LOG_INFO, "\x1B[36mthread %d: %lu hashes, %.2f H/s\x1B[0m", thr_id,
        ...
        if (opt_benchmark && thr_id == opt_n_threads - 1) {
            ...
            if (i == opt_n_threads) {
                switch(opt_algo) {
                case ALGO_CRYPTONIGHT:
                    applog(LOG_INFO, "\x1B[36mTotal: %s H/s\x1B[0m", hashrate);
                    break;
                default:
    ...
    while (1) {
        ...
        if (jsonrpc_2) {
            if (stratum.work.job_id
                ...
                time(&g_work_time);
                pthread_mutex_unlock(&g_work_lock);
                applog(LOG_INFO, "\x1B[35mNew job at diff %i\x1B[0m", (int)global_diff);
                restart_threads();
            }
        } else {
        ...
        if (!stratum_socket_full(&stratum, 120)) {
            applog(LOG_ERR, "\x1B[31mStratum connection timed out\x1B[0m");
            s = NULL;
        } else
            s = stratum_recv_line(&stratum);
        if (!s) {
            stratum_disconnect(&stratum);
            applog(LOG_ERR, "\x1B[31mStratum connection interrupted\x1B[0m");
            continue;
        }
